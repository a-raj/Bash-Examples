...
# touch fileName : it creates the file with the given file name

# mkdir : creates a new directory

# cd : change directory

# cat > test :  create acat file and opens it to write

# Cltr+D is end of document 

# cat < test : this will open the file and show whats in it

# cat fileName1 fileName2 > mergedFileName : two merge files merge into one

# mv fileName NewFileName : rename the file

# rm fileName : remove the file, also capable of removing directory

# rm -r directoryName :  remove directoty 
# rmdir : same as rm -r 

# cp fileName dirAddress : copy file to location

# Linkes : 2 kind 
# soft one and hard one

# ln fileName anotherFile : it will create a hard link that is a replica of a file(fileName) i.e changes in one file will refelect in another file wont be deleted if one is deleted

# ln -s old old_soft : this will create the soft link of the original file (kind of shortcuts) 


# permissions read (4),  wite (2) and execute (1) all have ceratin numeric values : total numeric value will be 7

# umask : 4 ditig number, when a file is created what unix does is it substract the umask from value 666 for a file and 777 for a dir

# ls -l : long listing, shows file along with its permissionsions
# - indicates the normal file 
# d indicates the directory
# file owner groups, and others i.e three group of read write and execute

# ls -a shows all the files that is system file also

# chmode 777 fileName : 4  for read 2 for write and 1 for excute this will grand owner, group and other permissions for the rwx for all 3

# uname -a : this will give information about the system -a stand for the all the informaton

##############################

# wc command 
# file * : all the files in the dir :  all the contents of home folder
It tells about what type of file you have ie file encoding and type

# wc fileName : first it tells about number of line, 2nd no of words and 3rd the number of alphabets in your test file.
# Wc -l : just print the line
-w : number of words
-c :  number of characters


### Short command
sort fileName : it will show the alphabetcal shorted o/p of file
sort : then it will wait for the input for the which it will sort

### cut command
cut : default it will treat tab as a delimeter
cut -d"-" -f 1,3 fileName : -d mean delimeter and then provide the delimeter -f to tell extract the fields then provide the fiels number that after seperating delimeter
...
##### man command : short for manual command
man is the help repository for the command in unix
man commandName : it will open all manual for the command


#### bannder command
 banner "bye bye" : it will display 10 char in a line 
 banner bye bye it will splits the bye bye and shows it

#### compress command
compress -v fileName : -v is for shell to show the %age of compression

## To decompress
uncompress fileName.z

### to see the compress file
zcat fileName
